...
export default function BostonSuburbsPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#FAF8F5] text-[#2D3436] font-body">
      ...
      <header className="sticky top-0 z-50 bg-white border-b border-[#E8E4DF] shadow-sm">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <button onClick={() => scrollToForm('header')} className="bg-[#1B4332] text-white font-semibold text-sm px-4 py-2 rounded-lg">Request Free Estimate</button>
          </div>
        </div>
      </header>

      <section ref={heroRef} className="relative overflow-hidden text-white">
        <Image src="/images/boston-suburbs/hero-colonial-fence.png" alt="Premium white vinyl fence at a colonial home in a Boston suburb" fill priority className="object-cover brightness-[0.45]" />
        <div className="absolute inset-0 bg-gradient-to-b from-[#0d2818]/90 via-[#1B4332]/88 to-[#0d2818]/95" />
      ...
      {mobileBannerVisible && <div className="fixed bottom-0 left-0 right-0 z-50 lg:hidden bg-white border-t border-[#E8E4DF] shadow-2xl"><div className="flex"><a href={PHONE_HREF} onClick={() => handlePhoneClick('mobile_bar')} className="flex-1 flex items-center justify-center gap-2 bg-[#1B4332] text-white font-bold text-sm py-4"><PhoneIcon />Call Now</a><button onClick={() => scrollToForm('mobile_bar')} className="flex-1 bg-[#C9A84C] text-white font-bold text-sm py-4">Free Estimate →</button></div><div className="bg-white" style={{ height: 'env(safe-area-inset-bottom)' }} /></div>}
