...
const GALLERY = [
  { src: '/images/wood/wood-horizontal-stained-gate.jpg', caption: 'Horizontal wood fence with a stained gate entry' },
  { src: '/images/vinyl/vinyl-privacy-white-gate.jpg', caption: 'White vinyl privacy fence with matching gate' },
  { src: '/images/pool/pool-aluminum-backyard-01.jpg', caption: 'Code-compliant aluminum pool enclosure' },
  { src: '/images/aluminum/aluminum-vinyl-mixed-yard.jpg', caption: 'Mixed aluminum and vinyl perimeter fencing' },
  { src: '/images/wood/wood-spindle-top-01.jpg', caption: 'Wood fence with a decorative spindle-top design' },
  { src: '/images/vinyl/vinyl-privacy-white-02.jpg', caption: 'Semi-privacy vinyl fence along a landscaped yard' },
];
...
export default function BostonSuburbsPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#FAF8F5] text-[#2D3436] font-body">
      ...
      <header className="sticky top-0 z-50 bg-white border-b border-[#E8E4DF] shadow-sm">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <button onClick={() => scrollToForm('header')} className="bg-[#1B4332] text-white font-semibold text-sm px-4 py-2 rounded-lg">Request My Private Consultation</button>
          </div>
        </div>
      </header>
      ...
      <section className="py-10 sm:py-14 px-4 bg-white border-t border-[#E8E4DF]"><div className="max-w-3xl mx-auto flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left"><div className="w-20 h-20 rounded-full bg-[#1B4332] text-white flex items-center justify-center font-heading font-bold text-2xl flex-shrink-0">TS</div><div><p className="font-heading font-bold text-[#1B4332] text-lg">Tiago Silva, Owner</p><p className="text-[#2D3436] text-sm leading-relaxed mt-1">&ldquo;Every property I work on gets the same attention I&apos;d want for my own home — I&apos;m personally involved from the first walkthrough to the final gate hinge.&rdquo;</p></div></div></section>
      ...
      <section className="py-12 sm:py-16 px-4 bg-white border-t border-[#E8E4DF]"><div className="max-w-6xl mx-auto"><div className="text-center mb-8"><h2 className="font-heading text-2xl sm:text-3xl font-bold text-[#1B4332]">Craftsmanship in the Details</h2><p className="text-[#9A9590] text-sm mt-2">Real JT Fence installations across Massachusetts.</p></div><div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">{GALLERY.map((item, index) => <button key={item.src} onClick={() => { gtagEvent('gallery_click_boston_suburbs', { image: index + 1, page: '/boston-suburbs' }); scrollToForm(`gallery_${index + 1}`); }} className="relative rounded-xl overflow-hidden bg-[#E8E4DF] aspect-[4/3] group"><Image src={item.src} alt={item.caption} fill className="object-cover group-hover:scale-105 transition-transform" sizes="(max-width: 640px) 50vw, 33vw" /><span className="absolute inset-0 bg-gradient-to-t from-[#1B4332]/70 via-[#1B4332]/0 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-3"><span className="text-white text-xs font-semibold leading-snug">{item.caption}</span></span></button>)}</div></div></section>
