...
interface BostonSuburbsQuoteFormProps {
  onSuccess?: () => void;
  onFormStart?: () => void;
}
...
export default function BostonSuburbsQuoteForm({
  ...
  const field = 'w-full px-3 py-3 border rounded-lg font-body text-[#2D3436] text-base placeholder-[#9A9590] focus:outline-none focus:border-[#C9A84C] focus:ring-1 focus:ring-[#C9A84C] transition-colors';
  const fieldClass = (name: string) => `${field} ${errors[name] ? 'border-red-400' : 'border-[#E8E4DF]'}`;

  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div><input name="name" value={formData.name} onChange={handleChange} placeholder="Your name" aria-label="Full name" className={fieldClass('name')} />{errors.name && <p className="text-red-500 text-xs mt-1">{errors.name}</p>}</div>
        <div><input type="tel" name="phone" value={formData.phone} onChange={handleChange} placeholder="Phone number" aria-label="Phone number" className={fieldClass('phone')} />{errors.phone && <p className="text-red-500 text-xs mt-1">{errors.phone}</p>}</div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div><input type="email" name="email" value={formData.email} onChange={handleChange} placeholder="Email address" aria-label="Email address" className={fieldClass('email')} />{errors.email && <p className="text-red-500 text-xs mt-1">{errors.email}</p>}</div>
        <div><input name="city" value={formData.city} onChange={handleChange} placeholder="Town or ZIP code" aria-label="Town or ZIP code" className={fieldClass('city')} />{errors.city && <p className="text-red-500 text-xs mt-1">{errors.city}</p>}</div>
      </div>
      <div>
        <select name="serviceType" value={formData.serviceType} onChange={handleChange} aria-label="Fence type" className={fieldClass('serviceType')}>
          <option value="">Fence type / service needed</option>
          {FENCE_TYPES.map((type) => <option key={type} value={type}>{type}</option>)}
        </select>
